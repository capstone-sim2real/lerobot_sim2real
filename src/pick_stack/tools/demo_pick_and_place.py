...
from pick_stack.control.grasp import biased_grasp_xy, plan_grasp_attempts
from pick_stack.control.ik import IkResult, TopDownIK
...
def highest_reachable_hover(ik, x_mm: float, y_mm: float, base_z_mm: float, cfg) -> float:
    ...
    while z >= floor:
        if ik.solve(x_mm, y_mm, z).position_error_mm <= 3.0:
            return z
        z -= cfg.motion.hover_search_step_mm
    return floor
...
def plan(cfg: AppConfig, color: str, dest_name: str, snapshot_url: str, points_csv: Path):
    ...
    det_x, det_y = float(xy_mm[0]), float(xy_mm[1])
...
def execute(cfg: AppConfig, p: dict) -> bool:
    ...
    from pick_stack.control.grasp import run_grasp_attempts
