...
Grasp trials (label each trial, then read the two distributions apart):
    ...
    python -m pick_stack.tools.tune_gripper_load --mode grasp --csv /tmp/grasp.csv
...
Passive watch (hand-move the arm, press the gripper on a block, watch loads):
    ...
    python -m pick_stack.tools.tune_gripper_load --mode watch --hz 5
...
from pick_stack.control import So101RobotIO, check_grasp
