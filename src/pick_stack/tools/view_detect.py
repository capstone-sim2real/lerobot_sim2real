"""Detection debug CLI: run the detector on a frame and save an annotated view.
    ...
    python -m pick_stack.tools.view_detect \
        --camera /dev/video0 \
        --calib src/pick_stack/configs/calib/lab.json \
        --out /tmp/detect.png

Use --image for saved frames, --set to try thresholds without editing YAML:

    python -m pick_stack.tools.view_detect --image frame.png --calib lab.json \
...
from pick_stack.perception import PlaneCalibration, detect_blocks, select_target, target_id_for
from pick_stack.tools._capture import grab_frame
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.image:
        ...
        cam_cfg = cfg.robot.cameras.get(cfg.perception.top_camera_key, {})
        frame = grab_frame(cam, width=cam_cfg.get("width", 640), height=cam_cfg.get("height", 480))
