...
def main():
    ...
    pdf.setTitle("Steel / Sky - Calm v2, Team Sim2Real, 14 slides")
    pdf.setAuthor("Team Sim2Real")
    manifest = {
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "generator": "built-in image_gen",
        "revision": "calm v2; slide 01 retained, slides 02-14 edited",
        "approved_cover_preserved": True,
    ...
    (ROOT / "README.md").write_text("""# Steel / Sky 발표 이미지 14장 / 차분한 v2
...
""")
    ...
    with ZipFile(archive, "w", ZIP_DEFLATED) as zipped:
        for file in sorted(ROOT.rglob("*")):
            if file.is_file() and file.suffix not in {".pptx", ".pdf"} and "drafts" not in file.relative_to(ROOT).parts:
