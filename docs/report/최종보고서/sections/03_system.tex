...
두 번째 단계에서는 특정 시작 위치에서 목표 지점까지 옮기는 시연을 중심으로 51개 에피소드를 수집하였다. 이 조건에서 단일 블록 이동은 50회 중 29회(58\%) 성공하였다. 위치를 한정하면 수행이 나아지는 것을 보고 시작 위치를 다시 전 영역으로 넓혔으며, 약 120개 에피소드로 ACT와 SmolVLA를 학습하였다. 그러나 이때의 수행 성공률은 약 10--20\%로 다시 낮아졌다. 세 단계 모두 블록 하나를 집어 옮기는 과제였으며, 다중 블록 조작으로 확장한 실험은 아니었다.

\begin{table}[htbp]
\centering
\caption{초기 단일 블록 모방학습의 단계별 조건과 팀 집계}\label{tab:early}
...
표~\ref{tab:early}는 팀이 최종 정리한 실험 순서와 집계이다. 51개 시연과 29/50회는 제공된 집계값이며, 나머지 수집량과 성공률 범위는 당시 결과에 대한 팀의 회고에 따라 근사치로 표기하였다. 수집량을 누적 합계로 더하지 않았으며, 성공률 범위를 모델별 확정 수치로 나누지 않았다. 단계별 작업 범위와 학습 조건이 달라 데이터 수 증가만의 효과로 해석할 수 없고, 단일 블록 이동 성공률을 최종 CV+IK의 다섯 블록 미션 완료율과 직접 비교하지 않았다.

\subsection{ACT와 CV+IK의 하이브리드 시도}
단일 블록의 파지와 이동에서도 성공이 불안정하자, 학습 정책이 전체 동작을 담당하는 부담을 줄이기 위해 ACT와 CV+IK의 하이브리드 방식을 시도하였다. 파지에는 ACT를 사용하고 배치는 IK로 연결하는 역할 분담을 검토·시험하였다. SmolVLA 대신 ACT를 선택한 이유는 팀의 비교 실험에서 추론이 더 빠르고 수행 성공률도 일관되게 높았기 때문이다. 이는 당시 장비와 학습 조건에서의 선택 근거이며, 모델별 추론 시간의 정확한 측정표나 일반적인 벤치마크 우열을 제시하는 것은 아니다.
...
\subsection{실행을 먼저 만들고 학습 자료로 기록하는 전환}
전환을 결정한 직접적인 계기는 카메라 위치 변화와 시연 재수집 부담이었다. 카메라를 조금 옮긴 뒤 기존 정책의 수행이 달라져 시연부터 다시 수집해야 했으며, 100개 이상의 에피소드를 사람이 다시 녹화하는 것은 큰 부담이었다. 같은 수집·학습·평가 과정을 반복해야 하는 상태로는 실제 시연장으로 장비를 옮긴 뒤의 수행도 기대하기 어려웠다.

팀은 이때 개발 순서를 바꾸기로 하였다. 크기와 색상이 정해진 블록과 고정 작업 평면에 맞추어 CV+IK 실행기를 먼저 만들고, 그 실행을 녹화하여 모방학습의 시연 데이터로 사용하는 것이다. 이 구상은 당장의 과제 수행과 이후 학습 자료 확보를 함께 겨냥하였다. CV+IK 제어를 구현·보완한 뒤 성공 실행을 저장하는 Task3으로 연결했으며, 저장한 데이터로 정책을 재학습하는 단계는 아직 수행하지 않았다.
...
카메라 변경 후 반복 시연 수집 부담 & 조정한 실행기의 성공 동작을 Task3으로 기록 \\
...
Task3은 CV+IK 전환 당시 구상한 실행 기반 시연 수집을 구현한 것이다. 초기 ACT·SmolVLA 실험에서는 리더암으로 에피소드를 하나씩 녹화했고, 카메라 위치 변화 뒤 이 작업을 반복해야 했다. CV+IK 실행을 개선한 뒤에는 센서값과 검출 결과로 대상 선택·파지·배치를 판정하는 같은 루프에서 관측·동작을 기록하여, 리더암 조작 없이 학습용 에피소드를 수집하도록 연결하였다.
