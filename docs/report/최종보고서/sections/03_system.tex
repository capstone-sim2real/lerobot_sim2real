...
\subsection{시연 수집과 학습 및 추론 실험}
초기에는 리더암의 시연으로 블록 조작 동작을 학습하도록 설계하였다. ACT는 여러 시점의 행동을 청크 단위로 예측하는 모방학습 방법이며\cite{act}, SmolVLA는 영상·언어 입력을 행동 생성에 연결하는 모델이다\cite{smolvla}. LeRobot으로 시연 수집과 학습·추론 환경을 구성하고, 로봇 제어기와 정책 서버는 gRPC 원격 추론으로 연결하였다.

중간 단계까지 초록 31개, 노랑 20개, 파랑 31개, 기본 색상 30개 등 112개 시연 에피소드를 수집하였다. 이 가운데 초록·노랑 51개로 SmolVLA를 8,000스텝 파인튜닝하였다. SmolVLA는 팔을 목표 근처까지 옮겼지만 단일 블록 시험 10회에서 파지 성공을 얻지 못했다. ACT는 좁은 위치 범위의 단일 블록 이동에서 액션 청크 갱신과 혼합 옵션을 조정한 뒤 50회 중 29회 성공하였다.

작업 범위를 넓히고 최대 다섯 블록에 대응하도록 ACT 학습 데이터를 131개 에피소드로 확대했으나, 해당 시험의 성공 집계는 50회 중 13회였다. 파지는 ACT, 배치는 IK로 처리하는 하이브리드 구조도 시험했으며 79개 에피소드로 학습한 파지는 50회 중 10회 성공하였다. 데이터 확대와 역할 분담 조정에도 과제 수행에 필요한 안정성을 확보하는 데 어려움이 남았다.

\begin{table}[htbp]
\centering
\caption{초기 모방학습의 시도와 팀 집계}\label{tab:early}
\begin{tabularx}{\linewidth}{@{}L{0.21\linewidth}YL{0.23\linewidth}@{}}
\toprule 접근법 & 시도한 조건 & 성공 / 시행 \\
\midrule
SmolVLA & 51에피소드, 8,000스텝, 단일 블록 & 0/10회 \\
단일 블록 ACT & 제한된 위치 범위, 청크 갱신·혼합 조정 & 29/50회 (58\%) \\
다중 블록 ACT & 131에피소드, 범위 확대와 최대 5블록 대응 & 13/50회 (26\%) \\
ACT+IK & 79에피소드, ACT 파지와 IK 배치 연결 & 파지 10/50회 (20\%) \\
\bottomrule
\end{tabularx}
\end{table}

표~\ref{tab:early}는 팀 내부 공유 기록에 근거한 개발 단계별 결과이다. 서로 다른 작업 범위와 학습 조건을 사용했으며 개별 시행의 위치·방향·체크포인트를 함께 대조할 자료는 확보하지 못했다. 따라서 수치 차이를 데이터 수 증가의 효과로 해석하거나, 최종 CV+IK의 다섯 블록 미션 완료율과 직접 비교하지 않았다.

\subsection{현장 운용을 고려한 전환 결정}
실기에서는 카메라 위치·각도와 조명 등 관측 조건이 달라질 때 동작 결과가 변하는 현상을 관찰하였다. 목표 근처에 도달한 팔이 빈손으로 닫히는 경우에도 원인이 입력 영상, 정책 출력 또는 실제 접촉 중 어느 단계에 있는지 구분하기 어려웠다. 그 결과 다음 실험에서 데이터 수집 조건, 학습 설정과 제어 방식 중 무엇을 바꿀지 판단하는 데 시간이 소요되었다.

과제는 장비를 시연장으로 옮긴 뒤에도 제한시간 내 여러 블록을 처리해야 했다. 당시 확보한 결과만으로는 환경이 달라진 상황에서 정책의 재현성을 충분히 신뢰하기 어려웠으며, 남은 개발 기간에는 현장에서 실패 원인을 확인하고 조정할 수 있는 구조가 필요하다고 판단하였다.

이에 크기와 색상이 정해진 블록, 고정된 작업 평면이라는 조건을 활용해 CV+IK로 전환하였다. CV는 블록 중심과 방향을, 캘리브레이션은 로봇 좌표를, IK는 후보 자세와 도달성을 제공한다. 그리퍼 위치·부하는 파지 여부를 확인하고 FSM은 재시도와 대상 전환을 관리한다. 중간 결과를 분리하면 검출 실패, 좌표 편차, 계획 불가와 빈손 파지를 각각 점검할 수 있다.

\begin{table}[htbp]
\centering
\caption{관찰한 제약과 전환 후의 설계 대응}
\begin{tabularx}{\linewidth}{@{}YY@{}}
\toprule 초기 운용에서의 제약 & CV+IK의 대응 \\
\midrule
관측 조건 변경 시 결과 변동 & 카메라 고정, 좌표 변환과 색·형상 조건의 개별 점검 \\
실패 원인의 분리 어려움 & 검출·좌표·IK·센서·상태 전이의 중간 결과 기록 \\
반복 과제 중 개별 파지 실패 & 파지 확인 뒤 운반, 회전 재시도와 대상 재선택 \\
수동 시연 수집의 반복 부담 & 성공 실행을 기록하는 Task3으로 후속 확장 \\
\bottomrule
\end{tabularx}
\end{table}

기존 모터 통신과 장비 제어, 파지 확인 구조는 재사용하였다. 전환의 기준은 실기 관찰, 진단 가능성과 개발 일정이며 특정 모델의 일반적인 한계를 입증한 것은 아니다. CV+IK에서도 환경 변화에 맞춘 캘리브레이션과 검출 조정이 필요하다. 두 방법의 일반화 성능을 동일 조건에서 비교하는 실험은 수행하지 않았다.
\FloatBarrier

\section{최종 시스템 아키텍처}
\subsection{구성 요소와 관측 경로}
이 절은 저장소의 기본 실행 경로와 실기에서 추가로 적용한 기능을 구분한다. 파지·배치·기록의 기본 구조는 현재 실행 코드로 확인하였다. ArUco 위치 추정과 손목 카메라 색상 게이트는 팀이 실험용 구현으로 실제 제어에 사용했으나, 보고서 정리 시점의 main에는 반영되지 않았다. 초기 위치 보정 후보와 접촉 적재 설계는 개발 이력으로 설명하며 현재 기본값을 과거 실험에 소급하지 않는다.
...
대응점 하나를 기록할 때 같은 블록 위치에서 두 장면을 촬영하였다. 먼저 그리퍼 기준점을 블록 윗면 중앙에 맞추고 관절값과 접촉 자세 영상을 저장한다. 블록을 그대로 둔 채 팔을 물린 뒤 윗면이 드러난 영상에서 픽셀 중심을 찾는다. 첫 장면의 관절값으로 구한 FK 좌표와 두 번째 장면의 영상 좌표를 같은 점으로 연결한다. 그림~\ref{fig:calibration-pair}는 9월 8일 대응점 P4를 이용한 수집 절차의 예이다. 최신 9월 13일 적합에 사용한 점과는 다른 기록이다.
...
\end{subfigure}
\caption{9월 8일 대응점 P4의 캘리브레이션 수집 과정. 두 사진은 원본의 같은 범위를 확대하였고, 오른쪽 표시는 기록한 블록 중심이다. 픽셀값은 원본 영상 기준, FK는 접촉 자세의 관절값으로 계산한 베이스 좌표이며 실제 턱 위치의 독립 측정값은 아니다.}
...
좌표 변환은 적합 RMS와 최대 LOO 오차로 평가하였다. 개발 목표는 각각 5\,mm 미만과 8\,mm 미만이다. 초기 대응점 확대 이후에도 9월 8일, 11일과 13일에 각각 아홉 점을 재수집하였다. 최신 결과는 RMS 5.15\,mm와 최대 LOO 13.53\,mm이며 두 기준에 미달한다. 실험별 수집 조건과 오차는 표~\ref{tab:calibration-history}에 날짜와 함께 제시한다. 파지 성공 여부는 좌표 적합과 별도로 센서 및 실기 결과로 평가한다.

카메라 안정성은 기준 영상과 현재 영상의 대응 코너 이동량으로 평가한다. 개별 코너의 검출 변동에 민감한 최댓값 대신 p95를 주요 지표로 사용하며, 설계 기준은 10분간 2\,pixel 미만이다. 보관된 9월 8일 검사는 약 1분의 단축 검사로, 최신 카메라 배치의 장시간 안정성을 검증한 자료는 아니다.
\FloatBarrier
\section{역기구학과 궤적 실행}
...
초기에는 좌표 편차를 흡수하려고 반경·접선 방향의 경험적 보정과 네 위치 후보 $(0,15)$, $(-15,0)$, $(0,-15)$, $(15,0)$\,mm를 사용하였다. 9월 8일 진단 기록에는 이러한 후보 변경과 대상 재선택이 남아 있다. 위치 후보를 늘려도 빈손 파지가 반복되거나 한 블록에 긴 시간이 소요되어, 이후 캘리브레이션과 파지 방향을 함께 점검하였다.
...
관제 오버레이와 런타임 IK는 같은 정사각형 대칭 방향 선택 함수를 사용한다. 파지 직전 실제 턱 위치를 영상으로 추정해 XY를 닫힌 루프로 보정하는 기능은 현재 main의 기본 실행 경로에 포함하지 않는다. 후보 수와 각도 변경의 효과는 당시 조건의 파지·시간 기록과 함께 평가해야 한다.
...
PICK에서는 기본 후보와 회전 재시도 후보의 IK 도달성을 검사한 뒤 실행 가능한 후보를 시도한다. 빈손으로 닫히면 남은 후보로 넘어가며, 후보를 소진하거나 개별 동작의 시간 제한에 걸리면 재선택 또는 순회 보류로 이어진다. PICK 내부의 후보 변경과 FSM의 대상 재선택은 서로 다른 복구 단계이다(그림~\ref{fig:pick-check}).
...
\end{tikzpicture}
\caption{PICK 내부의 후보 시도와 센서 판정. $\bar p$와 $\overline{|L|}$은 위치와 절대 부하의 표본 평균이며, 표시한 임계값은 현재 설정이다. 개별 동작 시간 초과는 PICK 실패로 처리하며, 외부 180초 종료와 구분한다.}
...
손목 카메라는 다시 장착하여 관제 영상과 데이터 수집에 활용하였다. 추가 실험에서는 손목 영상에서 대상 색상이 검출되지 않으면 파지를 시작하지 않는 게이트를 실제 제어에 적용하였다. 이는 팀이 확인한 실험용 구현이며 현재 main의 기본 파지 경로에는 반영되지 않았다. 기본 경로의 상단 영상·위치·부하 판정과 이 추가 게이트를 구분하여 설명한다.
...
\end{subfigure}
\caption{손목 카메라 재장착과 관제 화면. 별도 실험용 구현에서는 대상 색상 게이트를 실제 파지 제어에 적용했으며, 그림 자체는 장착·표시 상태를 보여준다.}
...
2차 미션은 1차 미션의 대상 선택·파지·확인 흐름을 공유하고 목적지를 한 적재 좌표로 바꾼다. 최종 계획기는 등록 영역 안의 좌표와 캘리브레이션의 파지 높이, 명목 블록 높이로 층별 접근·해제 자세를 IK로 계산한다. 현재 기본 실행은 접촉 하강을 비활성화하고, 운반 단계에서 도달한 자세에서 그리퍼를 열어 다음 순회로 넘어간다.

개발 초기에는 기록한 관절 키프레임과 부하 변화로 접촉을 감지하는 하강 전략을 구성하였다. 해당 전략과 접촉 중심의 설계 목표는 현재 직접 해제 경로와 구분한다. 명목 높이와 배치 횟수로 만든 계획은 실제 타워 높이·안정성의 측정값이 아니므로, 블록의 안착과 해제 후 5초 유지는 별도로 관찰해야 한다.

팀 확인에 따르면 최종 적재 실험도 현재 main과 같은 직접 해제 방식을 사용하였다. 4장은 5단 목표의 20회 시험에서 수행 도중 도달한 높이를 제시한다. 다섯 블록 적재라는 전체 미션, 수행 중의 최대 높이와 종료 후 유지 상태를 구분한다.
...
로봇 I/O 경계에 \texttt{RecordingRobotIO} 데코레이터를 두었다. 명령 직전의 관측과 안전 제한을 거쳐 실제 전송된 액션을 한 프레임으로 묶어 LeRobotDataset의 관절 상태, 설정한 카메라 영상, 액션과 작업 지시 형식으로 저장한다. PICK/VERIFY/TRANSPORT 코드는 1차 미션과 같다. 초기에는 안전 제한 전의 요청 액션이 기록되었으나, 이를 실제 전송값으로 수정하였다.

Task3은 home$\to$탐지$\to$파지$\to$운반$\to$배치$\to$home 복귀를 한 에피소드로 기록하며, 센서 파지 확인을 통과하고 운반·배치 동작과 이어지는 home 복귀까지 끝난 사이클을 저장한다. 개별 에피소드의 실제 안착 여부를 독립 영상 판정한 라벨과는 구분한다. 1차/2차 미션과 달리 그리퍼 회전이나 위치 보정 재시도 없이 최초 파지 1회만 시도하도록 제한하여, 실패 후 복구 동작이 섞이지 않은 깨끗한 시연만 남도록 설계하였다. 지정 영역 밖의 블록이 5초간 탐지되지 않으면 한 라운드가 끝난 것으로 보고 작업자에게 블록 재배치를 요청한다.
...
그리퍼에 ArUco 마커를 부착하고 위치 추정을 실제 제어 실험에 적용하였다. 손목 카메라 게이트와 마찬가지로 팀의 실험용 구현이며 현재 main에는 반영되지 않았다. 적용 전후의 독립적인 턱 위치 오차와 기능별 성공률은 측정하지 않아, 마커의 효과를 정량적인 개선량으로 제시하지 않는다. 추정 좌표를 자동 수집 데이터의 필드로 기록하고 검증하는 작업은 후속 과제로 남았다.
